	...
	for (const { file } of tempFramesFrames) {
		...
		switch (type.n) {
			...
			case 3:
				...
				break
			case 4:
				height = Math.max(1, Math.floor(height - (delH + 3)))
                width = Math.max(1, Math.floor(width - (delW + 0)))
				break
            case 5:
                height = Math.max(1, Math.floor(height - (delH - 1)))
                break
		}
